...
from iseteam.trips.models import Trip, HotelCheckIn, BusCheckIn, PayTrip, Confirmation, Room, ImageTrip, GalleryTrip, PaymentAssignment
...
def hotel(request, tripID, confirmation, roomID):
	...
	if not confirmation.has_room and confirmation.payment.is_paid:
		...
		if room.available_rooms <=0:
			...
			room.save()
			hotel_mail(roomID)
		return HttpResponse(json.dumps({'done':'yes','room':room.name,'fullname':'%s %s' % (confirmation.payment.name, confirmation.payment.last_name)}), content_type='application/json')
	else:
		return HttpResponse(json.dumps({'done':'no'}), content_type='application/json')
...
def payment_records(request, tripID):
	...
	staff_list = PayTrip.objects.filter(trip=trip,is_paid=True).values('staff').annotate(Count('staff')).order_by('staff')
	sales = []
	for v in staff_list:
		if not v['staff'] == None:
...
def see_history(request,paytripID):
	...
	html = "<table class='table'><thead><tr><th>Last change made by...</th><th>Date</th></tr></thead><tbody class='text-left'><tr>%s</tr></tbody></table>" % tr_content
...
def buses_html(request, tripID):
	...
	for bus in trip.buses.filter(is_full=False):
		html += ("<option value=" + str(bus.pk) +">" + bus.name + ":::::" + str(bus.available_seats) + " Left" + "</option>")
	return HttpResponse(html)
...
def rooms_html(request, tripID):
	...
	for room in rooms:
		html += ("<option value=" + str(room.pk) +">" + room.name + ":::::" + str(room.available_rooms) + " Left" + "</option>")
	return HttpResponse(html)
