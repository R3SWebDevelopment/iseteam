...
class Trip(models.Model):
	...
	def get_price_zeros(self):
		return int(self.price_sale * 100)
	...
	def get_absolute_url(self):
		if self.slug:
			return ('view_trip', (self.slug,))
		return ('view_trip', (self.id,))

	@models.permalink
	def get_queretaro_url(self):
		if self.slug:
			return ('view_trip_queretaro', (self.slug,))
		return ('view_trip_queretaro', (self.id,))
	...
	def get_buses_grouped(self):
		from iseteam.trips.models import BusCheckIn
...
class GalleryTrip(models.Model):
	...
	def get_absolute_url(self):
		if self.trip.slug:
			return ('view_album', (self.trip.slug,))
		return ('view_trip', (self.trip.id,))
...
class HotelCheckIn(models.Model):
	...
	def __unicode__(self):
		return u'%s,%s,%s,%s' % (self.name1,name2,name3,name4)
...
class PayTrip(models.Model):
	...
	def get_payment_records(self):
		return 'http://isefamily.com/trips/%s/payments/' % self.trip.pk
